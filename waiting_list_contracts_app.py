...
def get_blocked_statuses(days: int = 90) -> set:
    conn = get_conn()
    rows = conn.execute("SELECT status_name FROM status_settings WHERE blocks_service_for_days >= ?", (days,)).fetchall()
    conn.close()
    return {row['status_name'] for row in rows}
...
def run_ddl():
    ...
    with conn:
        ...
        cur.execute("""CREATE TABLE IF NOT EXISTS status_settings (status_name TEXT PRIMARY KEY, prevents_new_request INTEGER DEFAULT 0, blocks_service_for_days INTEGER DEFAULT 0, is_final_state INTEGER DEFAULT 0, FOREIGN KEY (status_name) REFERENCES request_statuses(name))""")
        ...
        if cur.execute("SELECT COUNT(1) FROM status_settings").fetchone()[0] == 0:
            open_statuses = {"طلب غير مكتمل","جاري دراسة الطلب ومراجعة الأوراق", "جارِ المعاينة", "يجب استيفاء متطلبات التعاقد", "قيد الانتظار", "مقبول"}
            blocked_statuses = {"مرفوض", "إرجاء التعاقد"}
            final_statuses = {"إعادة تقديم","مقبول", "مرفوض", "مغلق", "إرجاء التعاقد", "لا يوجد حاجة للتعاقد"}
...
def login_ui():
    ...
    logo_path = next((p for p in [Path("static/logo.png"), Path("static/logo.jpg")] if p.exists()), None)
    if logo_path:
    ...
    with st.form("login_form"):
        ...
        if st.form_submit_button("تسجيل الدخول"):
            ...
            hospital_user = conn.execute("SELECT * FROM hospitals WHERE username=? AND password_hash=?", (username, hashed_pw)).fetchone()
            if hospital_user:
                st.session_state.user = { "role": "hospital", **dict(hospital_user) }
                st.success("تم تسجيل الدخول بنجاح")
                st.rerun()

            admin_user = conn.execute("SELECT * FROM admins WHERE username=? AND password_hash=?", (username, hashed_pw)).fetchone()
            if admin_user:
                st.session_state.user = { "role": admin_user["role"], **dict(admin_user) }
                st.success("تم تسجيل الدخول بنجاح")
                st.rerun()
...
def hospital_home():
    ...
    if Path("static/logo.png").exists():
        st.sidebar.image("static/logo.png", width=80)
    ...
    selection = st.sidebar.radio("القائمة", list(menu_options.keys()) + ["🚪 تسجيل الخروج"])
...
def hospital_dashboard_ui(user: dict):
    ...
    with st.form("edit_hospital_profile"):
        ...
        with col3:
            license_start = st.date_input("بداية الترخيص", value=pd.to_datetime(hospital['license_start']).date() if hospital['license_start'] else None)
        ...
        with col4:
            license_end = st.date_input("نهاية الترخيص", value=pd.to_datetime(hospital['license_end']).date() if hospital['license_end'] else None)
...
def hospital_new_request_ui(user: dict):
    ...
    if submitted:
        ...
        # === تحديث مهم: منع التقديم لنفس الخدمة في حالات محددة ===
        # الحالات التي تمنع تقديم طلب جديد لنفس الخدمة
        prevented_statuses = {
            "جاري دراسة الطلب ومراجعة الأوراق",
            "جارِ المعاينة",
            "يجب استيفاء متطلبات التعاقد",
        ...
        if hospital_has_open_request(user["id"], service_id, prevented_statuses):  # تغيير hospital_id إلى id
            st.error("لا يمكن إنشاء طلب جديد لنفس الخدمة قبل إغلاق الطلب الحالي من قبل الإدارة.")
            return
            
        if hospital_blocked_from_request(user["id"], service_id, blocked_statuses):  # تغيير hospital_id إلى id
        ...
        cur.execute("""
            ...
            VALUES (?,?,?,?,?)
        """, (user["id"], service_id, age_category, "طلب غير مكتمل", datetime.now().isoformat()))  # تغيير hospital_id إلى id
        ...
        # ***تحديث مهم: تعيين الطلب النشط وعلامة التحرير النشط***
        st.session_state["active_request_id"] = req_id
        st.session_state[f"editing_request_{req_id}"] = True # تم الدخول في وضع التحرير تلقائيًا
        # ***تحديث مهم: إعادة تحميل الصفحة لتحديث الواجهة***
        st.rerun() # إعادة التحميل لعرض واجهة رفع المستندات

    # ***تحديث مهم: التحقق مما إذا كان هناك طلب نشط***
    req_id = st.session_state.get("active_request_id")
    if req_id:
        # ***تحديث مهم: التحقق مما إذا كان المستخدم في وضع التحرير النشط***
        is_currently_editing = st.session_state.get(f"editing_request_{req_id}", False)
        if is_currently_editing:
            ...
            documents_upload_ui(req_id, user, is_active_edit=False)
...
def render_file_downloader(doc: sqlite3.Row, key_prefix: str):
    if doc["file_path"]:
        try:
...
def request_details_ui(request_id: int, role: str = "hospital"):
    ...
    # ***تحديث مهم: السماح بالحذف في تبويب طلباتي***
    ...
    if (can_edit or can_delete) and role == "hospital":
        # === تحديث مهم: عرض زر الحذف بجوار زر التعديل ===
        col_del_edit, col_save_cancel = st.columns([1, 1])
        with col_del_edit:
            if can_delete and st.button("🗑️ حذف الطلب"):
                for d in docs:
                ...
                if f"editing_request_{request_id}" in st.session_state:
                    st.session_state.pop(f"editing_request_{request_id}", None)
                st.rerun()
            if can_edit and st.button("✏️ تعديل الطلب"):
...
def admin_home():
    user = st.session_state.user
    st.sidebar.image("static/logo.png", width=80) if Path("static/logo.png").exists() else None
    st.markdown("<div class='header'><h2>لوحة التحكم الإدارية</h2></div>", unsafe_allow_html=True)
    
    admin_menu = {
        "🏥 إدارة المستشفيات": admin_hospitals_ui,
        "🧾 إدارة الطلبات": admin_requests_ui,
        "📊 الإحصائيات": admin_statistics_ui,
        "🧩 إدارة الإعدادات": admin_lists_ui,
        "👥 إدارة المستخدمين": admin_users_ui,
        "📥 إدارة ملفات التنزيل": admin_resources_ui,
    }
    reviewer_menu = {
        "🧾 مراجعة الطلبات": admin_requests_ui,
        "📊 الإحصائيات": admin_statistics_ui,
    }
    
    menu = admin_menu if user["role"] == "admin" else reviewer_menu
    selection = st.sidebar.radio("القائمة", list(menu.keys()) + ["🚪 تسجيل الخروج"])
    ...
    if hospitals:
        ...
        with st.expander("🗑️ إدارة حذف المستشفى", expanded=False):
            ...
            if selected_hospital != "—":
                ...
                conn = get_conn()
                cur = conn.cursor()
                cur.execute("SELECT COUNT(*) as request_count FROM requests WHERE hospital_id = ? AND deleted_at IS NULL", (hospital_id,))
                request_count = cur.fetchone()['request_count']
                conn.close()
                        ...
                        with col1:
                            if st.button("✅ نعم، تأكيد الحذف", type="primary", key="confirm_delete"):
                                try:
                                    conn = get_conn()
                                    cur = conn.cursor()
                                    cur.execute("DELETE FROM hospitals WHERE id = ?", (st.session_state['delete_hospital_id'],))
                                    conn.commit()
                                    conn.close()
...
def edit_hospital_ui(hospital_id: int):
    ...
    with st.form("edit_h"):
        ...
        lic_start = st.date_input("بداية الترخيص", value=pd.to_datetime(h["license_start"]).date() if h["license_start"] else date.today())
        lic_end = st.date_input("نهاية الترخيص", value=pd.to_datetime(h["license_end"]).date() if h["license_end"] else date.today())
    ...
    with colA:
        ...
        if st.button("حفظ الحالة"):
            ...
            if is_final_status(new_status):
                ...
                cur.execute("UPDATE requests SET status=?, admin_note=?, closed_at=?, updated_at=? WHERE id=?", (new_status, note, closed_at, updated_at, request_id))
            else:
                if r['closed_at'] and not is_final_status(new_status):
                     cur.execute("UPDATE requests SET status=?, admin_note=?, closed_at=NULL, updated_at=? WHERE id=?", (new_status, note, updated_at, request_id))
                else:
                     cur.execute("UPDATE requests SET status=?, admin_note=?, updated_at=? WHERE id=?", (new_status, note, updated_at, request_id))
            conn.commit()
            conn.close()
            st.success("تم الحفظ")
            st.rerun()
    ...
    for d in docs:
        ...
        with c6:
            if st.button("حفظ", key=f"save_{d['id']}"):
                ...
                cur.execute("UPDATE documents SET required=?, satisfied=?, admin_comment=? WHERE id=?", (new_required_value, 1 if sat_toggle else 0, comment, d['id']))
                conn.commit()
                conn.close()
...
def admin_statistics_ui():
    ...
    with col2:
        services = [s['name'] for s in get_conn().execute("SELECT name FROM services WHERE active=1").fetchall()]
    ...
    for doc in doc_types:
        with st.expander(f"تعديل: {doc['display_name']} ({doc['name']})"):
            ...
            if st.button("حفظ التعديل", key=f"save_doc_{doc['name']}"):
                conn = get_conn()
                cur = conn.cursor()
                cur.execute("UPDATE document_types SET display_name = ?, is_video_allowed = ? WHERE name = ?", (new_display_name, 1 if new_is_video_allowed else 0, doc['name']))
                conn.commit()
                conn.close()
    ...
    with st.form("add_doc_type"):
        ...
        if st.form_submit_button("إضافة نوع مستند"):
            if new_doc_name and new_doc_display_name:
                ...
                cur = conn.cursor()
                try:
                    cur.execute("INSERT INTO document_types (name, display_name, is_video_allowed) VALUES (?, ?, ?)", (new_doc_name, new_doc_display_name, 1 if new_doc_is_video_allowed else 0))
                    conn.commit()
    ...
    with st.form("change_pw"):
        ...
        if st.form_submit_button("تغيير كلمة المرور", use_container_width=True):
            ...
            with conn:
                ...
                if user and user['password_hash'] == hash_pw(old_pw):
...
def main():
    ...
    if "user" not in st.session_state:
        ...
        if "welcome_shown" not in st.session_state:
            user_name = st.session_state.user.get("name", "admin")
