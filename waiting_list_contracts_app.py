...
def hash_pw(pw: str) -> str:
    # عند إنشاء تجزئة جديدة، استخدم دائمًا bcrypt
    return pwd_context.hash(pw, scheme="bcrypt")
            ...
            with get_conn() as conn:
                ...
                if user_data:
                    ...
                    if user_data['password_hash'] == old_hash_pw(password):
                        ...
                        # 2. إذا لم تكن التجزئة القديمة، حاول التحقق باستخدام bcrypt
                        try:
                            ...
                            # تجاهل الخطأ إذا كانت التجزئة غير معروفة (لأننا تحققنا من sha256 بالفعل)
                            pass
                
                # تأخير بسيط لمنع هجمات التوقيت
                if not ('user' in st.session_state):
                    pwd_context.hash("dummy_password_to_time_attack")
...
def admin_home():
    ...
    admin_menu = {
        "items": ["🏥 إدارة المستشفيات", "🧾 إدارة الطلبات", "📊 الإحصائيات", "📜 سجل النشاط", "🧩 إدارة الإعدادات", "👥 إدارة المستخدمين", "📥 إدارة ملفات التنزيل"],
        "icons": ["hospital", "card-list", "bar-chart-line", "clock-history", "gear", "people", "download"],
        "functions": [admin_hospitals_ui, admin_requests_ui, admin_statistics_ui, admin_activity_log_ui, admin_lists_ui, admin_users_ui, admin_resources_ui]
    }
    reviewer_menu = {
...
def admin_users_ui():
    ...
    with st.form("reset_password"):
        ...
        if user_type == "مستشفى":
            ...
            with get_conn() as conn:
                admins = conn.execute("SELECT id, username FROM admins WHERE username != 'admin' ORDER BY username").fetchall()
    ...
    with st.form("change_pw"):
        ...
        if st.form_submit_button("تغيير كلمة المرور", use_container_width=True):
            ...
            with get_conn() as conn:
                ...
                if user and pwd_context.verify(old_pw, user['password_hash']):
