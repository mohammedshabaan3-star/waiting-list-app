...
# لوحة الألوان الموحدة والجذابة للتطبيق
COLOR_PALETTE = {
    ...
    'white': '#ffffff',              # أبيض
    'gradient_start': '#2563eb',     # بداية التدرج
    'gradient_end': '#0ea5e9',       # نهاية التدرج
}
...
<style>
    ...
    /* === الصفحة الرئيسية === */
    .main {{
        background: linear-gradient(135deg, {COLOR_PALETTE['background']} 0%, {COLOR_PALETTE['background_alt']} 100%);
    ...
    h3 {{
        color: {COLOR_PALETTE['primary_accent']};
        font-weight: 650;
        font-size: 1.4rem;
    ...
    .stForm {{
        ...
        border-radius: 16px;
        padding: 2rem;
        background: linear-gradient(135deg, {COLOR_PALETTE['white']} 0%, {COLOR_PALETTE['background']} 100%);
    ...
    .stForm:hover {{
        border-color: {COLOR_PALETTE['primary_accent']};
    ...
    .stDownloadButton > button {{
        background: linear-gradient(135deg, {COLOR_PALETTE['success']} 0%, #15803d 100%);
        color: {COLOR_PALETTE['white']};
        width: 100%;
        font-weight: 700;
        font-size: 1.05rem;
    ...
    .stTabs [aria-selected="true"] {{
        border-bottom-color: {COLOR_PALETTE['primary_accent']};
        color: {COLOR_PALETTE['primary']};
    }}
    
    /* === الجداول - جذابة وواضحة === */
    .stDataFrame {{
        width: 100% !important;
    }}
    ...
    .stDataFrame th {{
        background: linear-gradient(135deg, {COLOR_PALETTE['primary']} 0%, {COLOR_PALETTE['primary_accent']} 100%) !important;
        color: {COLOR_PALETTE['white']} !important;
        font-weight: 800 !important;
        padding: 16px !important;
        text-align: right !important;
        border-bottom: 3px solid {COLOR_PALETTE['primary_dark']} !important;
        font-size: 0.98rem;
        line-height: 1.5;
    ...
    .stDataFrame td {{
        ...
        border-bottom: 2px solid {COLOR_PALETTE['border']} !important;
    ...
    .stMultiSelect > div > div:hover {{
        border-color: {COLOR_PALETTE['primary_accent']} !important;
    }}
    ...
    /* لوحة التحكم الإدارية - لون هادئ احترافي */
    ...
    /* صفحة تسجيل الدخول - لون أزرق فاتح هادئ */
    .login-header {{
        text-align: center;
        color: {COLOR_PALETTE['white']};
        font-weight: 800;
        ...
        border-radius: 16px;
        box-shadow: 0 6px 20px rgba(108, 153, 205, 0.15);
        font-size: 2rem;
        line-height: 1.4;
        text-shadow: none;
    }}
    
    /* تحسين وضوح العنوان في صفحة الدخول */
    .login-header h1 {{
        color: #000000;                                   /* أسود صريح */
        font-weight: 900;                                 /* خط عريض جداً */
        text-shadow: 0 2px 4px rgba(0, 0, 0, 0.2);       /* ظل خفيف ناعم */
        letter-spacing: 0.5px;                            /* تباعد طفيف للوضوح */
    }}
    ...
    .subheader {{
        ...
        border-bottom: 4px solid {COLOR_PALETTE['primary_accent']};
        padding-bottom: 1rem;
        font-size: 1.4rem;
        letter-spacing: -0.3px;
    }}
    ...
    .stats-item {{
        ...
        align-items: center;
        padding: 1.2rem 0;
        border-bottom: 2px solid {COLOR_PALETTE['background_alt']};
        gap: 1rem;
    }}
    ...
    .footer {{
        ...
        margin-top: 4rem;
        padding: 2.5rem;
        border-top: 3px solid {COLOR_PALETTE['primary_light']};
        color: {COLOR_PALETTE['text_light']};
        font-size: 0.95rem;
        background: linear-gradient(135deg, {COLOR_PALETTE['background']} 0%, {COLOR_PALETTE['background_alt']} 100%);
        line-height: 1.8;
    ...
    .stProgress > div > div > div {{
        background: linear-gradient(90deg, {COLOR_PALETTE['primary']} 0%, {COLOR_PALETTE['primary_accent']} 100%) !important;
        border-radius: 10px !important;
    }}
    ...
    .stSuccess {{
        background: linear-gradient(135deg, {COLOR_PALETTE['success_light']} 0%, {COLOR_PALETTE['background']} 100%) !important;
        border-left: 5px solid {COLOR_PALETTE['success']} !important;
        border-radius: 10px !important;
    ...
    .stWarning {{
        background: linear-gradient(135deg, {COLOR_PALETTE['warning_light']} 0%, {COLOR_PALETTE['background']} 100%) !important;
        border-left: 5px solid {COLOR_PALETTE['warning']} !important;
        border-radius: 10px !important;
        padding: 1.2rem !important;
    }}
...
@lru_cache(maxsize=1)
def _db_pragmas_tuple():
    # immutable tuple for cache key stability
    return (
        ("journal_mode", "WAL"),
        ("synchronous", "NORMAL"),
        ("cache_size", 10000),
...
# ---------------------------- طبقة تنفيذ الاستعلامات مع تخزين مؤقت ذكي ---------------------------- #
# ملاحظة: لا نغير نص الاستعلامات الموجودة. نضيف طبقة تنفيذ اختيارية قابلة لإعادة الاستخدام.

@st.cache_data(show_spinner=False, ttl=300, max_entries=256)
def cached_query(query: str, params: tuple = (), one: bool = False, tag: str = None):
    """تنفيذ استعلام SELECT بتخزين مؤقت مبني على (query, params, one, tag).
    - لا تستخدم إلا للاستعلامات القرائية فقط.
    - tag يسمح بتجميع منطقي لإبطال ذاكرة محددة لاحقاً.
    """
    with get_conn() as conn:
...
def invalidate_cache_tags(tags: list[str] | None = None):
    """تفريغ تخزين بيانات الاستعلامات عند تغيّر المدخلات.
    إذا لم تُحدد وسوم، يتم تفريغ كلي.
    """
    try:
        if not tags:
            cached_query.clear()
            return True
        # Streamlit cache لا يدعم إبطال جزئي مخصص، لذلك نفرّغ كلياً عند الحاجة.
        cached_query.clear()
        return True
    except Exception:
        return False
...
def ensure_notifications_table():
    ...
    يتضمن:
    - حقول أساسية: id, title, message, created_at, is_read
    - حقول الدور والقطاع: user_id, user_role, role, sector
    - حقول الكيان: entity_type, entity_id
    - حقول موروثة: type, related_id
    - عمود التكرار: event_key
    """
    with get_conn() as conn:
        conn.execute("""
        CREATE TABLE IF NOT EXISTS notifications (
            ...
            event_key TEXT DEFAULT NULL,         -- unique identifier for deduplication
        ...
        try:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_notifications_user_role ON notifications(user_role)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_notifications_entity_type ON notifications(entity_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_notifications_created_at ON notifications(created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_notifications_event_key ON notifications(event_key)")
        except Exception as e:
...
def create_notification(user_id: int = None,
    ...
    المعاملات:
    - user_id: معرف المستخدم المستهدف (اختياري)
    - user_role: دور المستخدم أو دور الهدف
    - title: عنوان الإشعار (مطلوب)
    - message: محتوى الإشعار (مطلوب)
    - notification_type: نوع الإشعار (يحفظ كـ type للتوافق)
    - entity_type: نوع الكيان (request / hospital / document / system)
    - entity_id: معرف الكيان المرتبط
    - sector: القطاع (اختياري)
    - event_key: مفتاح فريد لمنع التكرار (اختياري)
    
    العودة: True عند النجاح، False عند الفشل
    """
    try:
        # تطبيع الحقول أولاً
        title = str(title).strip()[:500] if title else ""  # حد أقصى 500 حرف
        message = str(message).strip()[:2000] if message else ""  # حد أقصى 2000 حرف
        
        # التحقق من الحقول الإلزامية بعد التطبيع
        if not title or not message:
            print(f"create_notification: missing required fields. title={title!r}, message={message!r}")
            return False
        
        # تطبيع نوع الكيان
        spec_entity_type = entity_type or notification_type or "system"
        if spec_entity_type not in ("request", "hospital", "document", "system"):
            spec_entity_type = "system"
        
        # التأكد من وجود جدول الإشعارات وعمود event_key
        try:
            _ensure_notifications_event_key_column()
        except Exception:
            pass  # لا نعطل العملية إذا فشل هذا
        
        now_iso = datetime.now().isoformat()
        
        with get_conn() as conn:
            # إدراج الإشعار مع جميع الحقول
            if event_key:
                conn.execute(
                    """
                    INSERT INTO notifications (
                        user_id, user_role, role, sector, title, message, 
                        entity_type, entity_id, type, related_id, event_key, 
                        is_read, created_at
                    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """,
                    (user_id, user_role, user_role, sector, title, message,
                     spec_entity_type, entity_id or related_id, notification_type, 
                     related_id, event_key, 0, now_iso)
                )
            else:
                conn.execute(
                    """
                    INSERT INTO notifications (
                        user_id, user_role, role, sector, title, message, 
                        entity_type, entity_id, type, related_id, 
                        is_read, created_at
                    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
                    """,
                    (user_id, user_role, user_role, sector, title, message,
                     spec_entity_type, entity_id or related_id, notification_type, 
                     related_id, 0, now_iso)
                )
            conn.commit()
            return True
    except Exception as e:
...
def _build_notifications_query_for_user(user_id: int, role: str, sector: str = None, unread_only: bool = False, limit: int = 50, offset: int = 0):
    ...
    # فقط أضف LIMIT و OFFSET إذا كانا معيّنة (limit > 0)
...
def get_user_notifications(user_id: int, user_role: str, user_sector: str = None, unread_only: bool = False, limit: int = 50, offset: int = 0):
    ...
    - user_role: دور المستخدم (admin, reviewer_general, reviewer_sector, hospital)
    - user_sector: قطاع المستخدم (للـ reviewer_sector)
    - unread_only: إرجاع الإشعارات غير المقروءة فقط
    - limit: حد أقصى للعدد المُرجَع (0 = بدون حد)
    - offset: عدد الإشعارات المراد تخطيها
    
    العودة: قائمة الإشعارات
    """
    try:
        with get_conn() as conn:
            query, params = _build_notifications_query_for_user(user_id, user_role, user_sector, unread_only, limit, offset)
            # استخدم التخزين المؤقت للاستعلامات
            notif_list = cached_query(query, tuple(params), one=False, tag="notifications_list") or []

            # إزالة الإشعارات المكررة بناءً على event_key أو entity
            seen = {}
            
            def get_fingerprint(n: dict) -> str:
                """حساب بصمة فريدة للإشعار لتجنب التكرار"""
                event_key = n.get('event_key')
                if event_key:
                    return f"ek::{event_key}"
                
                entity_type = n.get('entity_type') or ''
                entity_id = n.get('entity_id')
                if entity_type and entity_id:
                    return f"ent::{entity_type}:{entity_id}"
                
                # الخيار الأخير: بصمة العنوان والرسالة
                title = n.get('title') or ''
                message = n.get('message') or ''
                return f"txt::{title}:{message}"
            
            for notif in notif_list:
                fingerprint = get_fingerprint(notif)
                
                # الحصول على تاريخ الإنشاء للمقارنة
                current_dt = None
                try:
                    created_str = notif.get('created_at')
                    if created_str:
                        current_dt = datetime.fromisoformat(created_str)
                except (ValueError, TypeError):
                    current_dt = None
                
                if fingerprint not in seen:
                    # إضافة إشعار جديد
                    seen[fingerprint] = (notif, current_dt)
                else:
                    # الاحتفاظ بالإشعار الأحدث
                    existing_notif, existing_dt = seen[fingerprint]
                    try:
                        if current_dt and (existing_dt is None or current_dt > existing_dt):
                            seen[fingerprint] = (notif, current_dt)
                    except TypeError:
                        pass  # لا نقارن إذا فشلت المقارنة
            
            # استخراج الإشعارات المزالة من التكرار وترتيبها
            deduped = [v[0] for v in seen.values()]
            
            def get_datetime(n: dict):
                """محاولة الحصول على تاريخ الإشعار"""
                try:
            ...
            deduped.sort(key=get_datetime, reverse=True)
...
def mark_notification_read(notification_id: int, acting_user_id: int = None, acting_user_role: str = None, acting_user_sector: str = None):
    ...
    try:
        with get_conn() as conn:
            ...
            conn.commit()
            # إبطال التخزين المؤقت للإشعارات لضمان تحديث البيانات
            try:
                invalidate_cache_tags(["notifications_list"])
            except Exception:
                pass  # لا نعطل العملية إذا فشل إبطال التخزين المؤقت
            return True
    except Exception as e:
...
def get_unread_count(user_id: int, user_role: str, user_sector: str = None):
    ...
    try:
        # استعلام مباشر بدون تخزين مؤقت لضمان دقة البيانات
        with get_conn() as conn:
            # بناء الاستعلام
            if user_role == "admin":
                query = "SELECT COUNT(1) as c FROM notifications WHERE is_read = 0"
                params = []
            elif user_role == "reviewer_general":
                query = "SELECT COUNT(1) as c FROM notifications WHERE entity_type = 'request' AND is_read = 0"
                params = []
            elif user_role == "reviewer_sector":
                query = "SELECT COUNT(1) as c FROM notifications WHERE sector = ? AND is_read = 0"
                params = [user_sector]
            elif user_role == "hospital":
                query = "SELECT COUNT(1) as c FROM notifications WHERE user_id = ? AND is_read = 0"
                params = [user_id]
            else:
                query = "SELECT COUNT(1) as c FROM notifications WHERE (user_id = ? OR role = ?) AND is_read = 0"
...
def render_sidebar_notifications(user: dict):
    ...
    try:
        ...
        st.write("🔔 الإشعارات")
        if unread > 0:
            st.markdown(f"<span style='background-color: #ff6b6b; color: white; padding: 2px 8px; border-radius: 12px; font-weight: bold;'>📬 {unread} رسالة غير مقروءة</span>", unsafe_allow_html=True)
        else:
            st.caption("✅ لا توجد رسائل جديدة")
        ...
        with st.expander("عرض الإشعارات الأخيرة"):
            ...
            for n in notifs:
                try:
                    ...
                    # إجراء: تعليم كمقروء
                    if not is_read:
                        if st.button("مقروء", key=f"sidebar_mark_{notif_id}"):
                            try:
                                mark_notification_read(notif_id, acting_user_id=uid, acting_user_role=urole, acting_user_sector=usector)
                                st.success("تم تحديث الإشعار")
                                st.rerun()
                            except Exception as e:
                                print(f"Error marking read: {e}")
                                st.error("خطأ في تحديث الإشعار")
                    else:
                        st.caption("(مقروء)")
                    
                    st.divider()
                except Exception as e:
                    print(f"Error rendering notification {n.get('id')}: {e}")
                    st.warning(f"خطأ في عرض الإشعار #{n.get('id')}")
                    continue
    except Exception as e:
...
def get_list_from_meta(key: str, default_list: list) -> list:
    # قراءة بسيطة من meta عبر الكاش
    row = cached_query("SELECT value FROM meta WHERE key=?", (key,), one=True, tag="meta")
    return (row["value"].split(",") if row and row.get("value") else []) or default_list
...
def get_request_statuses() -> list:
    rows = cached_query("SELECT name FROM request_statuses ORDER BY id", (), one=False, tag="request_statuses")
...
def get_preventing_statuses() -> set:
    ...
    rows = cached_query("SELECT status_name FROM status_settings WHERE prevents_new_request = 1", (), one=False, tag="status_settings")
    return {row['status_name'] for row in rows}

@st.cache_data(ttl=600)
def get_blocking_statuses() -> set:
    """الحصول على الحالات التي تمنع التقديم لفترة معينة."""
    rows = cached_query("SELECT status_name FROM status_settings WHERE blocks_service_for_days > 0", (), one=False, tag="status_settings")
    return {row['status_name'] for row in rows}

def is_final_status(status: str) -> bool:
    row = cached_query("SELECT is_final_state FROM status_settings WHERE status_name = ?", (status,), one=True, tag="status_settings")
    return bool(row and row.get('is_final_state') == 1)

@st.cache_data(ttl=1800)
def get_editable_statuses_for_role(role: str) -> set:
    """احصل على الحالات التي يمكن لدور معين تعديلها"""
    if role == "admin":
        # الأدمن يمكنه تعديل جميع الحالات
        statuses = cached_query("SELECT name FROM request_statuses ORDER BY id", (), one=False, tag="request_statuses")
        return {s['name'] for s in statuses}
    else:
        # المراجعون يمكنهم تعديل فقط الحالات المسموح بها
        perms = cached_query("SELECT status_name FROM reviewer_status_permissions WHERE role_name = ?", (role,), one=False, tag=f"perms:{role}")
        return {p['status_name'] for p in perms}

def can_reviewer_modify_status(role: str, current_status: str, new_status: str) -> bool:
    """التحقق من أن المراجع يمكنه تعديل من الحالة الحالية إلى الحالة الجديدة"""
    if role == "admin":
        return True  # الأدمن يمكنه تعديل أي حالة
    
    # المراجعون يجب أن تكون الحالة الحالية والحالة الجديدة معاً مسموحة
    editable = get_editable_statuses_for_role(role)
    return current_status in editable and new_status in editable

@st.cache_data(ttl=1800)
def get_reviewer_status_permissions_matrix() -> dict:
    """احصل على مصفوفة جميع صلاحيات المراجعين لتعديل الحالات"""
    roles = ["reviewer_general", "reviewer_sector"]
    matrix = {}
    for role in roles:
        matrix[role] = list(get_editable_statuses_for_role(role))
    return matrix

@st.cache_data(ttl=1800)
def get_document_types() -> list:
    rows = cached_query("SELECT name, display_name, is_video_allowed FROM document_types ORDER BY name", (), one=False, tag="document_types")
...
def get_optional_docs_for_type(hospital_type: str) -> set:
    rows = cached_query("SELECT doc_name FROM hospital_type_optional_docs WHERE hospital_type = ?", (hospital_type,), one=False, tag=f"opt_docs:{hospital_type}")
...
def set_optional_docs_for_type(hospital_type: str, doc_names: list):
    ...
    invalidate_cache_tags(["document_types", f"opt_docs:{hospital_type}"])
...
def ensure_request_docs(request_id: int, hospital_type: str):
    ...
    with get_conn() as conn:
        existing_rows = conn.execute("SELECT doc_type FROM documents WHERE request_id=?", (request_id,)).fetchall()
        existing = {r["doc_type"] for r in existing_rows}
...
def index_exists(conn, index_name: str) -> bool:
    ...
    result = conn.execute("SELECT name FROM sqlite_master WHERE type='index' AND name=?", (index_name,)).fetchone()
    return result is not None

def foreign_key_exists(conn, table: str, column: str, ref_table: str) -> bool:
    """التحقق من وجود مفتاح خارجي على عمود"""
    try:
        # الحصول على معلومات الفهارس الخارجية للجدول
        fks = conn.execute(f"PRAGMA foreign_key_list({table})").fetchall()
        for fk in fks:
            if fk['from'] == column and fk['table'] == ref_table:
                return True
        return False
    except:
        return False

def verify_and_auto_migrate_schema():
    """
    التحقق الشامل من توافقية قاعدة البيانات وإنشاء العناصر المفقودة تلقائياً
    ✅ المبدأ الأول: عدم حذف أو تعديل البيانات القديمة - إضافة فقط
    ✅ المبدأ الثاني: إنشاء كل ما هو مفقود بصمت وآمان
    """
    try:
        ...
        with get_conn() as conn:
            ...
            # ==== الخطوة 1: إنشاء جدول meta إذا كان مفقوداً (ضروري للمتابعة) ====
            if not table_exists(conn, 'meta'):
                conn.execute("""CREATE TABLE IF NOT EXISTS meta (
                    `key` TEXT PRIMARY KEY, 
                    value TEXT
                )""")
                migration_log.append("✅ تم إنشاء جدول meta")
            
            # ==== الخطوة 2: التحقق من الجداول الأساسية وإنشاء المفقود ====
            required_tables = {
                ...
                'hospital_type_optional_docs': """CREATE TABLE IF NOT EXISTS hospital_type_optional_docs (
                    id INTEGER PRIMARY KEY, 
                    hospital_type TEXT NOT NULL, 
                    doc_name TEXT NOT NULL, 
                    UNIQUE(hospital_type, doc_name)
                )""",
                'request_statuses': """CREATE TABLE IF NOT EXISTS request_statuses (
                    id INTEGER PRIMARY KEY, 
                    name TEXT UNIQUE NOT NULL
                )""",
                'status_settings': """CREATE TABLE IF NOT EXISTS status_settings (
                ...
                'backup_settings': """CREATE TABLE IF NOT EXISTS backup_settings (
                    id INTEGER PRIMARY KEY,
                ...
                'reviewer_status_permissions': """CREATE TABLE IF NOT EXISTS reviewer_status_permissions (
                    id INTEGER PRIMARY KEY,
                    role_name TEXT NOT NULL,
                    status_name TEXT NOT NULL,
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL,
                    UNIQUE(role_name, status_name),
                    FOREIGN KEY (status_name) REFERENCES request_statuses(name) ON DELETE CASCADE
                )"""
            }
            ...
            for table_name, create_sql in required_tables.items():
                if not table_exists(conn, table_name):
                    ...
                    migration_log.append(f"✅ تم إنشاء جدول {table_name}")
            
            # ==== الخطوة 3: إضافة الأعمدة المفقودة بشكل آمن ====
            columns_to_add = {
                'admins': [('sector', 'TEXT'), ('updated_at', 'TEXT')],
                'hospitals': [
                    ('sector', 'TEXT'), ('governorate', 'TEXT'), ('address', 'TEXT'),
                    ('updated_at', 'TEXT'), ('other_branches', 'TEXT'),
                    ('other_branches_address', 'TEXT'), ('license_start', 'TEXT'),
                    ('license_end', 'TEXT'), ('manager_name', 'TEXT'),
                    ('manager_phone', 'TEXT'), ('license_number', 'TEXT')
                ],
                'requests': [
                    ('sector', 'TEXT'), ('governorate', 'TEXT'), ('updated_at', 'TEXT'),
                    ('deleted_at', 'TEXT'), ('closed_at', 'TEXT'), ('admin_note', 'TEXT')
                ],
                'documents': [
                    ('updated_at', 'TEXT'), ('is_video_allowed', 'INTEGER DEFAULT 0'),
                    ('display_name', 'TEXT'), ('required', 'INTEGER DEFAULT 1'),
                    ('satisfied', 'INTEGER DEFAULT 0'), ('admin_comment', 'TEXT'),
                    ('uploaded_at', 'TEXT')
                ]
            }
            ...
            for table_name, columns in columns_to_add.items():
                if table_exists(conn, table_name):
                    for col_name, col_type in columns:
                        # استخراج اسم العمود فقط (بدون DEFAULT أو غيره)
                        simple_col_name = col_name.split()[0]
                        if not column_exists(conn, table_name, simple_col_name):
                            try:
                                conn.execute(f"ALTER TABLE {table_name} ADD COLUMN {col_name}")
                                migration_log.append(f"✅ تمت إضافة العمود {simple_col_name} إلى {table_name}")
                            except Exception as e:
                                migration_log.append(f"⚠️ فشل إضافة {simple_col_name} إلى {table_name}: {e}")
            
            # ==== الخطوة 4: تحديث البيانات القديمة بحذر ====
            # تحديث sector و governorate للطلبات من بيانات المستشفى
            if table_exists(conn, 'requests') and table_exists(conn, 'hospitals'):
                if column_exists(conn, 'requests', 'sector') and column_exists(conn, 'hospitals', 'sector'):
                    try:
                        conn.execute("""
                            ...
                            WHERE sector IS NULL AND hospital_id IS NOT NULL
                        """)
                        migration_log.append("✅ تم تحديث بيانات sector للطلبات من hospitals")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل تحديث sector: {e}")
                ...
                if column_exists(conn, 'requests', 'governorate') and column_exists(conn, 'hospitals', 'governorate'):
                    try:
                        conn.execute("""
                            ...
                            WHERE governorate IS NULL AND hospital_id IS NOT NULL
                        """)
                        migration_log.append("✅ تم تحديث بيانات governorate للطلبات من hospitals")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل تحديث governorate: {e}")
            
            # ==== الخطوة 5: إنشاء الفهارس لتحسين الأداء ====
            indexes_to_create = [
                ("idx_requests_hospital_service", "CREATE INDEX IF NOT EXISTS idx_requests_hospital_service ON requests(hospital_id, service_id)"),
                ...
                ("idx_requests_deleted", "CREATE INDEX IF NOT EXISTS idx_requests_deleted ON requests(deleted_at)"),
                ("idx_requests_sector", "CREATE INDEX IF NOT EXISTS idx_requests_sector ON requests(sector)"),
                ("idx_requests_governorate", "CREATE INDEX IF NOT EXISTS idx_requests_governorate ON requests(governorate)"),
                ("idx_documents_request_id", "CREATE INDEX IF NOT EXISTS idx_documents_request_id ON documents(request_id)"),
                ("idx_documents_doc_type", "CREATE INDEX IF NOT EXISTS idx_documents_doc_type ON documents(doc_type)"),
                ("idx_documents_updated_at", "CREATE INDEX IF NOT EXISTS idx_documents_updated_at ON documents(updated_at)"),
                ("idx_hospitals_sector", "CREATE INDEX IF NOT EXISTS idx_hospitals_sector ON hospitals(sector)"),
                ("idx_hospitals_governorate", "CREATE INDEX IF NOT EXISTS idx_hospitals_governorate ON hospitals(governorate)"),
                ("idx_services_active_name", "CREATE INDEX IF NOT EXISTS idx_services_active_name ON services(active, name)")
            ]
            ...
            for idx_name, create_sql in indexes_to_create:
                try:
                    ...
                    migration_log.append(f"⚠️ تحذير في إنشاء فهرس {idx_name}: {e}")
            
            # ==== الخطوة 6: التحقق من البيانات الافتراضية وإضافتها إذا لزم الأمر ====
            # التحقق من admins
            if table_exists(conn, 'admins'):
                admin_count = conn.execute("SELECT COUNT(1) as cnt FROM admins").fetchone()['cnt']
                if admin_count == 0:
                    try:
                        default_password = "admin123"
                        conn.execute("INSERT INTO admins (username, password_hash, role, sector) VALUES (?,?,?,?)", 
                                   ("admin", hash_pw(default_password), "admin", None))
                        migration_log.append(f"✅ تم إنشاء مسؤول افتراضي (كلمة المرور: {default_password})")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل إنشاء المسؤول الافتراضي: {e}")
            
            # التحقق من services
            if table_exists(conn, 'services'):
                services_count = conn.execute("SELECT COUNT(1) as cnt FROM services").fetchone()['cnt']
                if services_count == 0:
                    try:
                        conn.executemany("INSERT INTO services (name) VALUES (?)", [(s,) for s in DEFAULT_SERVICES])
                        migration_log.append(f"✅ تم إضافة {len(DEFAULT_SERVICES)} خدمة افتراضية")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل إضافة الخدمات: {e}")
            
            # التحقق من document_types
            if table_exists(conn, 'document_types'):
                doc_types_count = conn.execute("SELECT COUNT(1) as cnt FROM document_types").fetchone()['cnt']
                if doc_types_count == 0:
                    try:
                        video_allowed = {"فيديو لغرف العمليات والإقامة"}
                        conn.executemany("INSERT INTO document_types (name, display_name, is_video_allowed) VALUES (?, ?, ?)", 
                                        [(d, d, 1 if d in video_allowed else 0) for d in DOC_TYPES])
                        migration_log.append(f"✅ تم إضافة {len(DOC_TYPES)} نوع وثيقة افتراضي")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل إضافة أنواع الوثائق: {e}")
            
            # التحقق من request_statuses
            if table_exists(conn, 'request_statuses'):
                statuses_count = conn.execute("SELECT COUNT(1) as cnt FROM request_statuses").fetchone()['cnt']
                if statuses_count == 0:
                    try:
                        conn.executemany("INSERT INTO request_statuses (name) VALUES (?)", [(s,) for s in DEFAULT_REQUEST_STATUSES])
                        migration_log.append(f"✅ تم إضافة {len(DEFAULT_REQUEST_STATUSES)} حالة طلب افتراضية")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل إضافة حالات الطلب: {e}")
            
            # التحقق من status_settings
            if table_exists(conn, 'status_settings'):
                settings_count = conn.execute("SELECT COUNT(1) as cnt FROM status_settings").fetchone()['cnt']
                if settings_count == 0 and table_exists(conn, 'request_statuses'):
                    try:
                        ...
                        conn.executemany("INSERT INTO status_settings VALUES (?, ?, ?, ?)", settings)
                        migration_log.append("✅ تم إضافة إعدادات الحالات الافتراضية")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل إضافة إعدادات الحالات: {e}")
            
            # التحقق من hospital_type_optional_docs
            if table_exists(conn, 'hospital_type_optional_docs'):
                optional_docs_count = conn.execute("SELECT COUNT(1) as cnt FROM hospital_type_optional_docs").fetchone()['cnt']
                if optional_docs_count == 0:
                    try:
                        docs_data = [("حكومي", d) for d in GOVERNMENT_OPTIONAL_DOCS] + [("خاص", d) for d in PRIVATE_OPTIONAL_DOCS]
                        conn.executemany("INSERT INTO hospital_type_optional_docs (hospital_type, doc_name) VALUES (?, ?)", docs_data)
                        migration_log.append(f"✅ تم إضافة {len(docs_data)} وثيقة اختيارية حسب نوع المستشفى")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل إضافة الوثائق الاختيارية: {e}")
            
            # التحقق من reviewer_status_permissions
            if table_exists(conn, 'reviewer_status_permissions'):
                perms_count = conn.execute("SELECT COUNT(1) as cnt FROM reviewer_status_permissions").fetchone()['cnt']
                if perms_count == 0 and table_exists(conn, 'request_statuses'):
                    try:
                        now_iso = datetime.now().isoformat()
                        general_reviewer_statuses = [s for s in DEFAULT_REQUEST_STATUSES if s not in ["مقبول", "مرفوض", "مغلق", "لا يوجد حاجة للتعاقد", "طلب غير مرسل"]]
                        
                        perms_data = []
                        for status in general_reviewer_statuses:
                            perms_data.append(("reviewer_general", status, now_iso, now_iso))
                        
                        sector_reviewer_statuses = ["جاري دراسة الطلب ومراجعة الأوراق", "جارِ المعاينة", "يجب استيفاء متطلبات التعاقد"]
                        for status in sector_reviewer_statuses:
                            perms_data.append(("reviewer_sector", status, now_iso, now_iso))
                        
                        conn.executemany("INSERT INTO reviewer_status_permissions (role_name, status_name, created_at, updated_at) VALUES (?, ?, ?, ?)", perms_data)
                        migration_log.append(f"✅ تم إضافة {len(perms_data)} صلاحية تعديل حالة افتراضية")
                    except Exception as e:
                        migration_log.append(f"⚠️ فشل إضافة صلاحيات الحالات: {e}")
            
            # ==== الخطوة 7: الحفظ والتحديث ====
            conn.commit()
            
            # تحديث إصدار Schema
            if current_version < DB_SCHEMA_VERSION:
                set_schema_version(DB_SCHEMA_VERSION)
                migration_log.append(f"✅ تم ترقية إصدار قاعدة البيانات من {current_version} إلى {DB_SCHEMA_VERSION}")
            
            return migration_log
    
    except Exception as e:
        print(f"❌ خطأ حرج في verify_and_auto_migrate_schema: {e}")
        return [f"❌ خطأ: {e}"]

def run_migrations():
    """تشغيل migrations آمنة لتحديث قاعدة البيانات تلقائياً - متوافق مع أي نسخة قديمة"""
    try:
        migration_log = verify_and_auto_migrate_schema()
        
        # طباعة السجل بالكامل
        for log_entry in migration_log:
            if log_entry:  # تجنب الأسطر الفارغة
                print(log_entry)
        
        return migration_log
    
    except Exception as e:
        print(f"⚠️ خطأ في run_migrations (سيتم المتابعة): {e}")
        return [f"⚠️ خطأ: {e}"]

def run_ddl():
    """
    ✅ تُستدعى مرة واحدة عند بدء التطبيق
    ✅ تتحقق من توافقية قاعدة البيانات وتنشئ العناصر المفقودة تلقائياً
    ✅ لا تحذف أو تعدل أي بيانات قديمة
    """
    try:
        run_migrations()
    except Exception as e:
        print(f"⚠️ خطأ في run_ddl: {e}")
...
def login_ui():
    ...
    st.markdown(f"<div class='login-header'><h1>{APP_TITLE}</h1></div>", unsafe_allow_html=True)
...
def get_active_services():
    rows = cached_query("SELECT id, name FROM services WHERE active=1 ORDER BY name", (), one=False, tag="services_active")
        ...
        with get_conn() as conn:
            ...
            try:
                ...
                cur.execute("""
                    ...
                    VALUES (?,?,?,?,?,?,?)
                """, (user["id"], service_id, age_category, "طلب غير مرسل", hospital_sector_value, hospital_governorate_value, datetime.now().isoformat()))
            except sqlite3.OperationalError:
                ...
                cur.execute("""
                    ...
                    VALUES (?,?,?,?,?)
                """, (user["id"], service_id, age_category, "طلب غير مرسل", datetime.now().isoformat()))
...
def documents_upload_ui(request_id: int, user: dict, is_active_edit: bool = False):
    ...
    # Mandatory messaging about document completion and visibility rules
    st.info("""
    📋 **متطلبات إرسال الطلب:**
    - ✅ يجب رفع **جميع المستندات المطلوبة** (المسماة 🔴)
    - ✅ بعد رفع المستندات، اضغط على زر **"إرسال الطلب"**
    - ✅ **بعد التقديم فقط، سيصبح الطلب مرئياً للإدارة والمراجعين**
    - ⏸️ الطلبات غير المكتملة تبقى خاصة بك فقط
    """)
    ...
    if is_active_edit:
        ...
        if st.button("إرسال الطلب", disabled=not all_required_uploaded, use_container_width=True):
            if not all_required_uploaded: 
                 st.error("❌ لا يمكن إرسال الطلب: هناك مستندات مطلوبة لم يتم رفعها.")
                ...
                st.success("✅ تم إرسال الطلب بنجاح. سيتم مراجعته من قبل الإدارة.")
                st.balloons()
                time.sleep(0.5)
                st.rerun()
        elif not all_required_uploaded:
            st.info("⚠️ يرجى رفع جميع المستندات المطلوبة (🔴) لتفعيل زر 'إرسال الطلب'. المستندات الاختيارية (🟡) غير ملزمة.")
...
def request_details_ui(request_id: int, role: str = "hospital"):
    """واجهة تفاصيل الطلب للمستخدم (المستشفى) - بدون زر الحذف للمستخدمين العاديين."""
    with get_conn() as conn:
        r = conn.execute("""
    ...
    can_edit = r['status'] in ["طلب غير مرسل", "جاري دراسة الطلب ومراجعة الأوراق", "يجب استيفاء متطلبات التعاقد"]

    # Regular users (hospital) cannot edit or delete after submission
...
def admin_home():
    ...
    st.markdown("<div class='admin-dashboard-header'><h2>لوحة التحكم الإدارية</h2></div>", unsafe_allow_html=True)
    
    admin_menu = {
        "items": ["🏥 إدارة المستشفيات", "🧾 إدارة الطلبات", "📊 الإحصائيات", "📜 سجل النشاط", "🔐 صلاحيات الحالات", "🧩 إدارة الإعدادات", "👥 إدارة المستخدمين", "💾 إدارة النسخ الاحتياطي", "📥 إدارة ملفات التنزيل", "🔑 تغيير كلمة المرور"],
        "functions": [admin_hospitals_ui, admin_requests_ui, admin_statistics_ui, admin_activity_log_ui, admin_status_permissions_ui, admin_lists_ui, admin_users_ui, admin_backup_ui, admin_resources_ui, lambda: change_password_ui(user_id=user["id"], user_table="admins")]
    }
    reviewer_general_menu = {
        "items": ["🧾 مراجعة الطلبات", "🔑 تغيير كلمة المرور"],
        "functions": [admin_requests_ui, lambda: change_password_ui(user_id=user["id"], user_table="admins")]
    }
    ...
    with st.sidebar:
        ...
        username = user.get('username', '')
        if user["role"] == "admin":
            st.markdown(f"**{username}** - admin")
        elif user["role"] == "reviewer_general":
            st.markdown(f"**{username}** - مراجع عام")
        elif user["role"] == "reviewer_sector":
            st.markdown(f"**{username}** - مراجع قطاع")
        else:
...
def _callback_update_request_status(request_id: int, new_status: str, note: str, current_status_is_final: bool):
    """Callback لتحديث حالة الطلب وملاحظة الأدمن مع التحقق من الصلاحيات."""
    try:
        # الحصول على معلومات المستخدم
        user = st.session_state.get("user", {})
        user_role = user.get("role")
        
        # الحصول على الحالة الحالية للطلب للتحقق من الصلاحيات
        with get_conn() as check_conn:
            req = check_conn.execute("SELECT status FROM requests WHERE id=?", (request_id,)).fetchone()
            current_status = req['status'] if req else None
        
        # التحقق من الصلاحية
        if not can_reviewer_modify_status(user_role, current_status, new_status):
            st.error(f"❌ ليس لديك صلاحية لتعديل الحالة من '{current_status}' إلى '{new_status}'")
            return
...
def _callback_soft_delete_request(request_id: int, docs: list):
    ...
    تصريح: Admin و General Reviewer فقط
    """
    # تحقق من صلاحية المستخدم قبل التنفيذ
    user = st.session_state.get("user", {})
    if user.get("role") not in ["admin", "reviewer_general"]:
        st.error("❌ ليس لديك صلاحية لتنفيذ هذا الإجراء. فقط الأدمن والمراجع العام مصرحون.")
        return
...
def admin_requests_ui():
    ...
    with status_col:
        status_options = ["الكل"] + [s for s in get_request_statuses() if s != "طلب غير مرسل"] + ["طلب غير مرسل"]
    ...
    if sector_filter:
        # تنظيف الفلتر من " AND " في البداية لضمان عدم حدوث خطأ في بناء الاستعلام
        clean_filter = sector_filter.strip()
        if clean_filter.upper().startswith("AND "):
            clean_filter = clean_filter[4:].strip()
    ...
    # Automatically filter out unsubmitted requests from admin/reviewers unless explicitly selected
    if status == "الكل":
        conditions.append("r.status != ?")
        params.append("طلب غير مرسل")
    elif status != "الكل":
        conditions.append("r.status = ?")
...
def admin_request_detail_ui(request_id: int):
    ...
    # منع الوصول إلى الطلبات غير المكتملة للمسؤولين والمراجعين
    if r['status'] == "طلب غير مرسل" and user.get("role") in ["admin", "reviewer_general", "reviewer_sector"]:
        st.error("❌ لا يمكن الوصول إلى الطلبات غير المُقدّمة. يمكن للمستخدم فقط عرض طلباته قبل التقديم.")
        return
    ...
    with colA:
        # احصل على الحالات المتاحة للدور الحالي
        editable_statuses = get_editable_statuses_for_role(user.get("role"))
        
        # اعرض جميع الحالات الحالية (الرؤية الكاملة)
        current_statuses = get_request_statuses()
        
        # لكن صفّي الخيارات المتاحة للاختيار (التعديل)
        available_statuses = [s for s in current_statuses if s in editable_statuses]
        
        # إذا لم تكن الحالة الحالية متاحة للتعديل، أضفها كخيار (للعرض فقط)
        if r['status'] not in available_statuses:
            available_statuses = [r['status']] + available_statuses
        
        # شكّل الخيارات مع تمييز الحالات غير المتاحة
        status_display = []
        for s in available_statuses:
            if s in editable_statuses:
                status_display.append(s)
            else:
                status_display.append(f"{s} (حالية - غير قابلة للتعديل)")
        
        new_status = st.selectbox("الحالة", status_display, 
                                 index=status_display.index(r['status']) if r['status'] in status_display else 0,
                                 help="يمكنك تعديل الحالة إلى الحالات المتاحة فقط حسب صلاحياتك")
        
        # أزل تمييز الحالات غير المتاحة عند الحفظ
        new_status_clean = new_status.split(" (")[0] if " (" in new_status else new_status
        ...
        st.button("حفظ الحالة", 
                  ...
                  args=(request_id, new_status_clean, note, current_is_final)
                 )
    ...
    # تحديد ما إذا كان المستخدم مصرحًا له بتنفيذ الإجراءات (Admin أو General Reviewer فقط)
    is_authorized_for_actions = user.get("role") in ["admin", "reviewer_general"]
    
    if not is_authorized_for_actions:
        st.warning("⚠️ ليس لديك صلاحية لتنفيذ هذه الإجراءات. فقط الأدمن والمراجع العام مصرحون.")
    ...
    with cols[0]:
        if is_authorized_for_actions:
            ...
            st.button("❌ حذف الطلب نهائيًا", disabled=True, 
                      help="فقط الأدمن والمراجع العام مصرحون"
                     )
    with cols[1]:
        def _callback_restore_request():
            # تحقق من الصلاحية قبل التنفيذ
            if not is_authorized_for_actions:
                st.error("❌ ليس لديك صلاحية لتنفيذ هذا الإجراء")
                return
            try:
                with get_conn() as conn:
        ...
        if is_authorized_for_actions:
            ...
            st.button("🔄 استرجاع كـ 'إعادة تقديم'", disabled=True,
                      help="فقط الأدمن والمراجع العام مصرحون"
                     )
    ...
    with cols[2]:
        ...
        if is_authorized_for_actions:
            ...
            st.button("🔒 إغلاق الطلب", disabled=True,
                      help="فقط الأدمن والمراجع العام مصرحون"
                     )
...
def admin_status_permissions_ui():
    """واجهة إدارة صلاحيات تعديل حالات الطلبات حسب الدور"""
    st.markdown("<div class='subheader'>🔐 إدارة صلاحيات تعديل حالات الطلبات</div>", unsafe_allow_html=True)
    ...
    st.info("""
    📌 من خلال هذه الصفحة، يمكنك التحكم في:
    - **من يمكنه تعديل أي حالة** من حالات الطلبات
    - الأدمن دائماً يمكنه تعديل جميع الحالات
    - المراجعون يمكنهم تعديل فقط الحالات التي تحددها هنا
    """)
    
    # الحصول على جميع الأدوار والحالات
    all_statuses = get_request_statuses()
    roles = ["reviewer_general", "reviewer_sector"]
    
    # تأكد من وجود الجدول وإنشاؤه إذا لم يكن موجوداً
    with get_conn() as conn:
        try:
            conn.execute("SELECT COUNT(1) FROM reviewer_status_permissions LIMIT 1")
        except:
            # إنشاء الجدول إذا لم يكن موجوداً
            conn.execute("""CREATE TABLE IF NOT EXISTS reviewer_status_permissions (
                id INTEGER PRIMARY KEY,
                role_name TEXT NOT NULL,
                status_name TEXT NOT NULL,
                created_at TEXT NOT NULL,
            ...
            now_iso = datetime.now().isoformat()
            general_reviewer_statuses = [s for s in all_statuses if s not in ["مقبول", "مرفوض", "مغلق", "لا يوجد حاجة للتعاقد", "طلب غير مرسل"]]
            for status in general_reviewer_statuses:
                conn.execute("INSERT OR IGNORE INTO reviewer_status_permissions (role_name, status_name, created_at, updated_at) VALUES (?, ?, ?, ?)", 
                           ("reviewer_general", status, now_iso, now_iso))
            
            sector_reviewer_statuses = ["جاري دراسة الطلب ومراجعة الأوراق", "جارِ المعاينة", "يجب استيفاء متطلبات التعاقد"]
            for status in sector_reviewer_statuses:
                conn.execute("INSERT OR IGNORE INTO reviewer_status_permissions (role_name, status_name, created_at, updated_at) VALUES (?, ?, ?, ?)", 
                           ("reviewer_sector", status, now_iso, now_iso))
            
            conn.commit()
            st.success("✅ تم إنشاء الجدول والبيانات الافتراضية بنجاح")
            st.rerun()
    
    # عرض مصفوفة الصلاحيات
    st.markdown("#### 📊 مصفوفة الصلاحيات الحالية")
    
    # قراءة الصلاحيات الحالية من قاعدة البيانات
    with get_conn() as conn:
        current_perms = conn.execute("SELECT role_name, status_name FROM reviewer_status_permissions").fetchall()
        perms_dict = {(p['role_name'], p['status_name']) for p in current_perms}
    
    # تنظيم الصلاحيات حسب الدور
    role_permissions = {}
    for role in roles:
        role_permissions[role] = {s: (role, s) in perms_dict for s in all_statuses}
    
    # عرض جدول قابل للتحرير
    tab1, tab2 = st.tabs(["تعديل الصلاحيات", "عرض الملخص"])
    
    with tab1:
        st.markdown("##### المراجع العام - تعديل الحالات المسموح بها")
        cols = st.columns([2] + [1] * min(len(all_statuses), 5))
        
        # رؤوس الأعمدة
        with cols[0]:
            st.markdown("**الحالة**")
        for i, status in enumerate(all_statuses[:5]):
            with cols[i + 1]:
                st.markdown(f"**{status[:15]}...**" if len(status) > 15 else f"**{status}**")
        
        # صفوف الحالات
        rows_display = []
        for idx, status in enumerate(all_statuses):
            is_allowed = role_permissions["reviewer_general"].get(status, False)
            rows_display.append((status, is_allowed))
        
        # نموذج للتعديل
        with st.form("edit_general_reviewer_permissions"):
            updated_perms = {}
            for status in all_statuses:
                current_val = role_permissions["reviewer_general"].get(status, False)
                allow = st.checkbox(f"السماح بـ: {status}", value=current_val, key=f"gr_{status}")
                updated_perms[status] = allow
            
            if st.form_submit_button("💾 حفظ صلاحيات المراجع العام"):
                with get_conn() as conn:
                    # حذف الصلاحيات القديمة
                    conn.execute("DELETE FROM reviewer_status_permissions WHERE role_name = ?", ("reviewer_general",))
                    
                    # إضافة الصلاحيات الجديدة
                    now_iso = datetime.now().isoformat()
                    for status, allow in updated_perms.items():
                        if allow:
                            conn.execute("INSERT INTO reviewer_status_permissions (role_name, status_name, created_at, updated_at) VALUES (?, ?, ?, ?)", 
                                       ("reviewer_general", status, now_iso, now_iso))
                    conn.commit()
                
                # مسح الذاكرة المؤقتة عند التحديث
                st.cache_data.clear()
                st.success("✅ تم حفظ الصلاحيات بنجاح")
                st.rerun()
        
        st.markdown("---")
        st.markdown("##### مراجع القطاع - تعديل الحالات المسموح بها")
        
        with st.form("edit_sector_reviewer_permissions"):
            updated_perms_sector = {}
            for status in all_statuses:
                current_val = role_permissions["reviewer_sector"].get(status, False)
                allow = st.checkbox(f"السماح بـ: {status}", value=current_val, key=f"sr_{status}")
                updated_perms_sector[status] = allow
            
            if st.form_submit_button("💾 حفظ صلاحيات مراجع القطاع"):
                with get_conn() as conn:
                    # حذف الصلاحيات القديمة
                    conn.execute("DELETE FROM reviewer_status_permissions WHERE role_name = ?", ("reviewer_sector",))
                    
                    # إضافة الصلاحيات الجديدة
                    now_iso = datetime.now().isoformat()
                    for status, allow in updated_perms_sector.items():
                        if allow:
                            conn.execute("INSERT INTO reviewer_status_permissions (role_name, status_name, created_at, updated_at) VALUES (?, ?, ?, ?)", 
                                       ("reviewer_sector", status, now_iso, now_iso))
                    conn.commit()
    ...
    with tab2:
        st.markdown("##### ملخص الصلاحيات")
        
        # عرض مصفوفة الصلاحيات الحالية
        matrix_data = []
        for role in roles:
            allowed_statuses = [s for s in all_statuses if role_permissions[role].get(s, False)]
            role_display = "المراجع العام" if role == "reviewer_general" else "مراجع القطاع"
            matrix_data.append({
                "الدور": role_display,
                "عدد الحالات المسموح بها": len(allowed_statuses),
                "الحالات المسموح بها": ", ".join(allowed_statuses) if allowed_statuses else "لا توجد"
            })
        ...
        st.markdown("""
        - **الأدمن**: دائماً يمكنه تعديل جميع الحالات بدون قيود
        - **المراجع العام**: يمكنه تعديل فقط الحالات المحددة أعلاه
        - **مراجع القطاع**: يمكنه تعديل فقط الحالات المحددة أعلاه
        - **الرؤية**: جميع الأدوار يرون جميع الحالات في الجدول (الرؤية الكاملة)
        - **التقييد**: التقييد ينطبق فقط على تعديل الحالة، وليس على عرض الحالة
        """)
