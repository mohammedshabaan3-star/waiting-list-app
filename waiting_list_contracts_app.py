    ...
    with get_conn() as conn:
        ...
        for dt in all_doc_types:
            ...
            if dt_name not in existing:
                ...
                docs_to_insert.append((request_id, dt_name, dt['display_name'], required, 0, None, dt['is_video_allowed']))
        if docs_to_insert:
            conn.executemany(
                "INSERT INTO documents (request_id, doc_type, display_name, required, satisfied, uploaded_at, is_video_allowed) VALUES (?,?,?,?,?,?,?)",
                docs_to_insert
            )
        
        # تحديث المستندات الموجودة بناءً على النوع الحالي للمستشفى
        for dt in all_doc_types:
            dt_name = dt['name']
            if dt_name in existing:
                required = 0 if dt_name in optional_docs else 1
                conn.execute("UPDATE documents SET required = ?, display_name = ?, is_video_allowed = ? WHERE request_id = ? AND doc_type = ?", 
