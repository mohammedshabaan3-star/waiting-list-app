...
def log_activity(action: str, details: str = "") -> None:
    ...
    try:
        ...
        st.error(f"خطأ في تسجيل النشاط: {e}")# الجزء الثاني من النسخة المحسنة - واجهات المستخدم
    ...
    with st.form("change_pw"):
        ...
        if st.form_submit_button("تغيير كلمة المرور", use_container_width=True):
            ...
            try:
                ...
                st.error(f"خطأ في تغيير كلمة المرور: {e}")# الجزء الثالث من النسخة المحسنة - صفحات الإدارة
...
def admin_statistics_ui() -> None:
    ...
    try:
        ...
        st.warning(f"حدث خطأ أثناء إنشاء الرسوم البيانية: {e}")# الجزء الرابع والأخير من النسخة المحسنة - باقي وظائف الإدارة والدالة الرئيسية
